...
export const createUserDetailsSv = asyncHandler(async (req: CustomExpressRequest, res) => {
  const { userId, email, gender, bod, departmentId, designationId } = req.body;
  const newData = await UserDetailsModel.create({ userId, email, gender, bod, departmentId, designationId });
  if (newData) {
    res.status(201);
...
export const inActiveUserSv = asyncHandler(async (req: CustomExpressRequest, res) => {
  ...
  const updatedData = await UserDetailsModel.update(
    { isActive: !isActive },
